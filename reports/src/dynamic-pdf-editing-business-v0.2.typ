...
We opened an R&D line to fix the poster-editing bug in NPC. The bug is not only about users uploading arbitrary third-party PDFs — it is about three different PDF provenance paths in our own pipeline, two of which break the editor. After intake, code audit, bug analysis, framework survey, and a deep-research pass on layout AI, we have a two-problem frame (preserve vs. synthesise), a four-approach matrix with primaries and kill criteria, and a cheapest-first experiment plan.
...
Most posters and infographics in scope are generated by #chip("npc-pr-agent") itself. The PDFs come from three paths, and two of them break the editor:

#v(0.3em)

#panel(title: "Three provenance classes, three failure modes", color: colors.primary)[
  #set text(size: 9pt)
  #table(
    columns: (1fr, 3.2fr, 1.6fr, 1.2fr),
    stroke: none,
    inset: (x: 6pt, y: 7pt),
    align: (center + horizon, left + horizon, left + horizon, center + horizon),
    table.header(
      table.cell(fill: colors.ink)[Class],
      table.cell(fill: colors.ink)[How the PDF gets made],
      table.cell(fill: colors.ink)[Text layer],
      table.cell(fill: colors.ink)[Editable],
    ),
    [A], [HTML → Playwright #chip("page.pdf()"). Intended path.], [Yes, selectable], status("partial", label: "partial"),
    table.hline(stroke: 0.3pt + colors.rule),
    [B], [PNG embedded into PDF via #chip("_png_to_pdf"). Silent regression.], [None, rasterised], status("risk", label: "no"),
    table.hline(stroke: 0.3pt + colors.rule),
    [C], [Externally uploaded third-party PDF.], [Depends on source], status("partial", label: "manual gate"),
  )
]

#callout(title: "The regression hidden in our own code", color: colors.danger, icon: icon-cross)[
  The comment at #chip("rendering_service.py:1054-1058") already spells out the intent: the exported PDF must retain selectable text or the field-discovery pipeline breaks. And yet eight export paths in #chip("orchestrator.py") still go via PNG embedding. Class B PDFs look right in a viewer but cannot be edited at all.
]

#v(0.3em)

This reframes the R&D as two problems, not one.
...
#grid(
  columns: (1fr, 1fr),
  gutter: 10pt,
  panel(title: "P1 — preserve what we already have", color: colors.success.darken(10%))[
    #set text(size: 9pt, fill: colors.ink-soft)
    For class-A PDFs, the pipeline already knows the field map at render time (Jinja placeholders in the HTML source). We throw it away at the PDF boundary. *Fix:* emit a sidecar or AcroForm at render time. Blast radius is one file. Cheap.
  ],
  panel(title: "P2 — synthesise the field map after the fact", color: colors.warning.darken(10%))[
    #set text(size: 9pt, fill: colors.ink-soft)
    For class-C uploads, and as a safety net for class A/B when the sidecar is missing, we recover editable fields from PDF bytes alone. The four approaches in the brief address this. Expensive.
  ],
)
...
#panel(title: "Approach matrix after Phase 2", color: colors.primary)[
  ...
  #table(
    ...
    [A detector finds "which bbox is a value, which is its label." A primitive, not an editor.],
    [Azure Document Intelligence primary; Gemini 2.5 Pro verifier],
...
#grid(
  ...
  callout(title: "Approach D resolved from deep research", color: colors.success.darken(15%), icon: icon-check)[
    Azure Document Intelligence as primary detector (real OCR geometry, inside our perimeter). Gemini 2.5 Pro as verifier on cropped low-confidence regions, not full pages. Semantic pairing of labels to values is ours to build.
  ],
)
