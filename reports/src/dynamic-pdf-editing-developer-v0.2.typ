...
The PDFs in scope come from three provenance classes: HTML-rendered via Playwright (intended, text-bearing), PNG-embedded via #chip("_png_to_pdf") (a silent regression, rasterised), and externally uploaded (rare, mixed). The incumbent #chip("PDFEditor") uses literal-text search against a human-curated #chip("PDFFieldDefinition") list; class A often fragments on it, class B has no text to search at all, class C was the original brief framing. This R&D addresses two problems: *preserve* the field map we already have at render time (cheap, additive to #chip("rendering_service")), and *synthesise* the field map after the fact when preservation is not possible (the four approaches in the brief). The #chip("PDFFieldDefinition") schema is already rich enough for either path.

= Provenance: how PDFs get into the editor today

#v(0.3em)

#panel(title: "Three pipelines, three failure modes", color: colors.primary)[
  #set text(size: 8.8pt)
  #table(
    columns: (0.7fr, 3.2fr, 1.5fr, 2fr),
    stroke: none,
    inset: (x: 6pt, y: 7pt),
    align: (center + horizon, left + horizon, left + horizon, left + horizon),
    table.header(
      table.cell(fill: colors.ink)[Class],
      table.cell(fill: colors.ink)[Source, code reference],
      table.cell(fill: colors.ink)[Text layer],
      table.cell(fill: colors.ink)[Why it breaks],
    ),
    [A], [HTML → Playwright #chip("page.pdf()"). See #chip("rendering_service.py:1004, 1062").], [Selectable], [Text present but often fragmented into many glyph runs; #chip("search_pattern") misses.],
    table.hline(stroke: 0.3pt + colors.rule),
    [B], [PNG embedded via #chip("_png_to_pdf"). See #chip("orchestrator.py:984, 1668, 1943, 2526, 2842, 3056, 3554").], [None], [No text at all. #chip("search_pattern") returns zero hits. Only fix: regenerate from source HTML or OCR.],
    table.hline(stroke: 0.3pt + colors.rule),
    [C], [Third-party uploaded PDF in a fill_poster flow.], [Depends on source], [No pre-registered template. Admin curation gate never ran. Plus: glyph fragmentation, outlined text, subsetted fonts, Arabic in visual order.],
  )
]

#callout(title: "Key finding during bug audit", color: colors.danger, icon: icon-cross)[
  The intent comment at #chip("rendering_service.py:1054-1058") says the exported PDF must retain selectable text "because the field-discovery pipeline needs real text spans, not a rasterised image." And yet eight #chip("orchestrator.py") export paths still go via PNG embedding. Class B is an architectural regression hidden inside normal-looking export code.
]
...
#grid(
  columns: (1fr, 1fr),
  gutter: 10pt,
  panel(title: "Problem P1 — sidecar at render time", color: colors.success.darken(10%))[
    #set text(size: 8.8pt, fill: colors.ink-soft)
    At render time the pipeline has the full field map (the Jinja placeholders in the source HTML). Persist it:
    #v(0.2em)
    - option 1: emit a sidecar JSON #chip("<uuid>.fields.json") next to the PDF with #chip("placeholder -> bbox"). No PDF-spec work.
    - option 2: write AcroForm fields into the PDF itself. Standards-compliant. Slight render-time cost.
    #v(0.2em)
    Blast radius: one file. No editor changes. Out-of-scope for this R&D (integration work), but the cheap fix for class A.
  ],
  panel(title: "Problem P2 — synthesise at edit time", color: colors.warning.darken(10%))[
    #set text(size: 8.8pt, fill: colors.ink-soft)
    For class C, and as a safety net for class A/B when no sidecar exists, recover editable fields from PDF bytes alone.
    #v(0.2em)
    This is what the four approaches in the brief address. E1–E4 in Phase 3 measure the viability of each.
  ],
)
...
#grid(
  ...
  panel(title: "E4. Layout-AI hybrid feasibility", color: colors.warning.darken(10%))[
    #set text(size: 9pt)
    - Input: rendered page images
    - Primary: Azure Document Intelligence Read/Layout
    - Verifier: Gemini 2.5 Pro on cropped candidate regions
    - Plus a semantic pairing stage (ours to build)
...
#grid(
  ...
  callout(title: "Arabic mixed-script reading order", color: colors.danger, icon: icon-cross)[
    Azure Document Intelligence documents that ambiguous reading order falls back to left-to-right, top-to-bottom. On Arabic posters with Latin numerals, that is the exact failure pattern we care about. E5 must probe logical-order vs ltr+ttb output, not just "does it read Arabic at all."
  ],
...
#panel(title: "Upstream change set under seam 2 + seam 1", color: colors.primary)[
  ...
  #table(
    ...
    [#chip("src/services/visual_content/services/rendering_service.py")], tag("p1", color: colors.success, variant: "solid"), [Emit a sidecar JSON #chip("placeholder -> bbox") at render time. Closes class-A bug cheaply.],
    table.hline(stroke: 0.3pt + colors.rule),
    [#chip("src/services/visual_content/orchestrator.py")], tag("fix", color: colors.danger, variant: "solid"), [Audit the 8 PDF-export sites; remove #chip("_png_to_pdf") regression where the html-native path exists. Eliminates class B.],
    table.hline(stroke: 0.3pt + colors.rule),
    [#chip("src/services/pdf/pdf_autotemplate.py")], tag("new", color: colors.success, variant: "solid"), [Synthesises #chip("(PDFTemplate, list[PDFFieldDefinition])") from upload bytes. Covers class C + class A/B without sidecar.],
    table.hline(stroke: 0.3pt + colors.rule),
