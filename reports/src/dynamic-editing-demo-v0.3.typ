...
#show: report.with(
  ...
  subtitle: "Phase-3 lab: overlay + UI. Latin + Arabic pass. Industry survey adds approach E.",
  date: "2026-04-25",
  version: "0.3",
...
#panel(title: "Approaches we evaluated", color: colors.primary)[
  ...
  #table(
    ...
    table.hline(stroke: 0.3pt + colors.rule),
    [C], [Overlay -- detect span, cover, redraw at same bbox in same font], [#text(fill: colors.success, weight: "bold")[chosen]], [Both Latin and Arabic working in the lab on both samples. SSIM \> 0.9998 on non-edited regions. Industry survey confirms this is the same primitive Apryse and Adobe use under the hood.],
    table.hline(stroke: 0.3pt + colors.rule),
    ...
    table.hline(stroke: 0.3pt + colors.rule),
    [E], [Localized diffusion glyph inpainting (AnyText2 / TextDoctor)], [#text(fill: colors.muted)[not started -- recommended]], [#strong[From industry survey.] Only genuinely novel candidate beyond A-D. Addresses C's failure modes: gradients, fragmented #chip("Tj") runs, heavily-subsetted fonts. Open as #chip("e8") spike (\~2 days). Open weights, GPU inference, font-conditioned, multilingual incl. Arabic.],
    table.hline(stroke: 0.3pt + colors.rule),
    [F], [Commercial WYSIWYG SDKs (Apryse / Nutrient / Foxit)], [#text(fill: colors.secondary, weight: "bold")[baseline-to-beat]], [Not novel -- same primitive as C, productized. Worth a 1-day Apryse WebViewer eyeball comparison; not worth licensing without measured win. Nutrient is LTR-only -- dead for Arabic.],
  )
]

#v(0.3em)

#panel(title: "Ruled out by industry survey -- variants of A-D or N/A on flat exports", color: colors.muted)[
  #set text(size: 8.6pt)
  #table(
    columns: (1.6fr, 3fr),
    stroke: none,
    ...
    table.header(
      table.cell(fill: colors.ink)[Approach],
      table.cell(fill: colors.ink)[Why we ruled it out],
    ),
    [Direct content-stream #chip("Tj") rewrite (Foxit, iText, pikepdf)],
    [Same primitive as C minus the redact step. Breaks on subset embeds, fragmented spans, Arabic shaping. Foxit's own docs warn the renderer breaks if new chars aren't in the embedded CIDFont.],
    table.hline(stroke: 0.3pt + colors.rule),
    [PDF → SVG → DOM-edit → PDF],
    [Inkscape forums confirm ligatures break and fonts substitute even when embedded. Arabic ligatures guaranteed to fail. Strict subset of B with worse fidelity.],
    table.hline(stroke: 0.3pt + colors.rule),
    [PDF → Markdown (Marker / Nougat) → render],
    [Re-rendered Markdown does not look like the original poster. Extraction-accurate, not visually faithful.],
    table.hline(stroke: 0.3pt + colors.rule),
    [ABBYY FineReader reconstructive edit],
    [Output is a regenerated lookalike, not the input PDF. Destructive same as B. Unlikely to beat overlay's 0.9998 SSIM.],
    table.hline(stroke: 0.3pt + colors.rule),
    [AcroForm / XFA fast-path],
    [Illustrator and Canva exports are flat -- no #chip("/AcroForm"), no #chip("/StructTreeRoot"). Cheap precondition check still worth adding for tagged-PDF future inputs.],
    table.hline(stroke: 0.3pt + colors.rule),
    [Closed-API multimodal image edit (gpt-image-1.5, Gemini)],
    [Same shape as E but closed-weights, no font conditioning, known regressions (masked-inpaint replaces entire image). Strictly weaker than E.],
    table.hline(stroke: 0.3pt + colors.rule),
    [Canva / Figma "import PDF as editable layers" plugins],
    [Black-box services. Useful only as manual-fallback if workflow ever pivots to user-edits-in-Figma. Skip for backend.],
  )
]
...
#panel(title: "Recommendation", color: colors.primary, keep: false)[
  ...
  + *Photographic backgrounds.* Will surface as soon as we test on the broader template library beyond the 90 samples shown in the prod template grid. Approach E is the natural fallback (see below).
...
#panel(title: "Suggested follow-up experiments (post-integration)", color: colors.warning)[
  #set text(size: 9pt)
  + *e8 -- localized diffusion inpainting* (\~2 days). Run AnyText2 with the embedded Lusail font as conditioning on a synthetic gradient-background poster + the two existing samples. Metric: masked-region SSIM + eyeball pass on glyph correctness. Decision threshold: SSIM \> 0.99 with pixel-correct numerals → adopt as v1 fallback when overlay fails.
  + *e9 -- Apryse WebViewer baseline* (\~1 day). Eyeball-compare commercial SOTA against our overlay on the two posters. Outcome is informational; do not license without measured win.
  + *e10 -- AcroForm precondition* (\~30 min). One-line check at upload: if the PDF has #chip("/AcroForm") or #chip("/StructTreeRoot"), fast-path through forms-aware fill instead of overlay.
  + *e3, e4* (the original gate-2 experiments) remain optional escape-hatches if uploaded posters ever break overlay in the wild.
]
...
docker run --rm -v "$PWD":/work --workdir /work \
  ...
  reports/src/dynamic-editing-demo-v0.3.typ \
  reports/out/dynamic-editing-demo-v0.3.pdf
```
...
- industry survey: `research/wiki/industry-landscape-survey.md` (vendor docs for Apryse / Nutrient / Foxit / Aspose / ABBYY; AnyText2, TextDoctor, DiffUTE papers; Inkscape SVG round-trip threads; OpenAI image-edit forum)
